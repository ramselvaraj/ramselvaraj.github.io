...
------
Here, the cost function we are trying to optimize will be Mean Squared Error(MSE), a function that will calculate the difference between the predicted value and the target value and square it. The value is squared to remove the sign dependancy and makes it easier to differentiate.
...
$$
\frac{dC}{dw} = \frac{dC}{da} X \frac{da}{dz} X \frac{dz}{dw}\\
\text{Where,}\\
